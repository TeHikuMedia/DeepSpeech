...
ModelState::ModelState()
    : beam_width_(-1), n_steps_(-1), n_context_(-1), n_features_(-1), mfcc_feats_per_timestep_(-1), sample_rate_(-1), audio_win_len_(-1), audio_win_step_(-1), state_size_(-1)
{
}
...
int ModelState::init(const char *model_path)
{
  return DS_ERR_OK;
}
  ...
  for (int i = 0; i < num_returned; ++i)
  {
    ...
    for (int j = 0; j < out[i].tokens.size(); ++j)
    {
      TokenMetadata token{
          ...
          static_cast<float>(out[i].probabilities[j])                    // probability
      };
