...
std::vector<std::string> Scorer::make_ngram(PathTrie* prefix)
{
  ...
  for (int order = 0; order < max_order_; order++) {
    ...
    std::vector<double> prefix_probs;

    if (is_utf8_mode_) {
      new_node = current_node->get_prev_grapheme(prefix_vec, prefix_steps, prefix_probs, alphabet_);
    } else {
      new_node = current_node->get_prev_word(prefix_vec, prefix_steps, prefix_probs, alphabet_);
    }
