...
typedef struct SentFitMeta{
  const SentFitToken *const tokens;
 ...
 */
  ...
  /**
 * @brief Returns metadata struct corresponding to pronunciation beam
 * 
 * @param aSctx A streaming state pointer returned by {@link DS_CreateStream()}.
 * @param target A char array containing target sentence to be pronounced.
 * 
 * @return The SentFitMeta stuct containing the pronounciation result. This contains:
 *          - char scores
 *          - char timing information
 *        The user is responsible for freeing the MetaData after inference
 *         {@link SF_FreeSentFitMeta()}.
 */
